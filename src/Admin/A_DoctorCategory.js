...
const A_DoctorCategory = () => {
    ...
    const getDCcategory = async () => {
        setloading(true)
        await axios({
            method: 'post',
            url: 'https://healtheasy-o25g.onrender.com/admin/doctorcategories/list',
            headers: {
                Authorization: token,
            },
            data: {
    ...
    const addDC = async () => {
        ...
        await axios({
            method: 'post',
            url: 'https://healtheasy-o25g.onrender.com/admin/doctorcategories/save',
            headers: {
                Authorization: token,
            },
            data: {
    ...
    function editDC(id){
        var datasingle = categorylist.filter((v, i) => { return v._id === id })
    ...
    const updateDC = async () =>{
        ...
        await axios({
            ...
            data: {
                categoryid: edit_record._id,
                categoryname: edit_record.categoryname
            }
        }).then((res) => {
            // toast('Surgery added...', { className: 'custom-toast-success' })
            Swal.fire({
    ...
    // columns data
        const columns = [{
            ...
            cell: row => <>
                ...
                <MdDelete onClick={() => deleteDC(row._id)} className='text-danger fs-5 ms-2' />
            </>
        }]
            ...
            <Container fluid className='p-0 panel'>
                    ...
                    <Col xs={12} sm={9} lg={10} className='p-3'>
                        ...
                        <div className='bg-white rounded p-3 shadow '>
                            <Row className='mt-2 mb-3 justify-content-between'>
                                ...
                                <Col xs={'auto'}>
                                    <Button variant='primary' onClick={handleDCshow}>Add Category</Button>
                                </Col>
                            </Row>

                            <DataTable columns={columns} data={categorylist? categorylist : ''} pagination />
                ...
                <Modal show={show_DC} onHide={handleDCclose} centered size="sm">
                    <Modal.Header closeButton>
                        <Modal.Title>Add Doctor Category</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Row className='g-4'>
                            <Col xs={12} md={12}>
                                    ...
                                    <Form className='row register_doctor'>
                                        <Form.Group controlId="name" className='mb-3 col-12'>
                                            <div className='position-relative'>
                                                ...
                                                <Form.Control placeholder="Enter Category Name" name="category" value={dc} onChange={(e) => setdc(e.target.value)} />
                                            </div>
                                        </Form.Group>

                                        <Form.Group className='col-12'>
                                            <Form.Control type='button' value={'Add Doctor Category'} onClick={addDC} className='theme_btn' />
                                        </Form.Group>
                                    </Form>
                                </div>
                            </Col>
                    ...
                    {/* <Modal.Footer>
                        <Button variant="secondary" onClick={handleDCclose}>
                            Close
                        </Button>
                    </Modal.Footer> */}
                </Modal>
                ...
                <Modal show={up_show_DC} onHide={handleupDCclose} centered size="sm">
                    <Modal.Header closeButton>
                        <Modal.Title>Update Doctor Category</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Row className='g-4'>
                            <Col xs={12} md={12}>
                                <div className='bg-white rounded p-3 shadow'>
                                    <Form className='row register_doctor'>
                                        <Form.Group controlId="name" className='mb-3 col-12'>
                                            <div className='position-relative'>
                                                <Form.Label>Category Name</Form.Label>
                                                <Form.Control placeholder="Enter Category Name" name="categoryname" value={edit_record?.categoryname} onChange={(e) => seteditrecord({...edit_record,categoryname:e.target.value})} />
                                            </div>
                                        </Form.Group>

                                        <Form.Group className='col-12'>
                                            <Form.Control type='button' value={'Update Doctor Category'} onClick={updateDC} className='theme_btn' />
                                        </Form.Group>
                                    </Form>
                                </div>
                            </Col>
...
export default A_DoctorCategory
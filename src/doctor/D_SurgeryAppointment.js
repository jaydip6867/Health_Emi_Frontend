...
const D_SurgeryAppointment = () => {
    ...
    const submitPrescription = async () => {
        ...
        try {
            ...
            Swal.fire('Success', 'Prescription saved and surgery appointment completed!', 'success')
            appointmentlist()
        ...
        <>
            ...
            <Container className='my-4'>
                                ...
                                <Modal.Body>
                                    <div className='p-2 rounded-3 border rounded' style={{ background: 'var(--white)' }}>
                                        ...
                                        <div>
                                            <Row className='g-3'>
                                                <Col md={6} xs={12}>
                                                    <div className='text-muted small mb-1'>Surgery Name</div>
                ...
                <Modal show={showStartAppointment} onHide={handleCloseStartAppointment} centered>
                    ...
                    <Modal.Footer>
                        <Button variant='secondary' onClick={handleCloseStartAppointment}>Close</Button>
                        <Button variant='primary' onClick={confirmStartAppointment} disabled={!currentAppointment}>Start</Button>
                    </Modal.Footer>
                </Modal>
                        ...
                        {currentAppointment ? (
                            <div className='d-flex flex-column gap-3'>
                                <div className='bg-light p-3 rounded'>
                                    <div className='row'>
                                        <div className='col-md-6'><strong>Patient:</strong> {currentAppointment.patientname}</div>
                                        <div className='col-md-6'><strong>Date:</strong> {currentAppointment.date}</div>
                                    </div>
                                    <div className='row mt-2'>
                                        <div className='col-md-6'><strong>Time:</strong> {currentAppointment.time}</div>
                                        <div className='col-md-6'><strong>Surgery:</strong> {currentAppointment?.surgerydetails?.name || '-'}</div>
                                    </div>
                                </div>
                                    ...
                                    <Row>
                                        <Col md={6}>
                                            <Form.Group className='mb-3'>
                                                ...
                                                <Form.Control type='text' placeholder='Enter BP...' value={prescriptionData.bp} onChange={(e) => handlePrescriptionChange('bp', e.target.value)} />
                                            </Form.Group>
                                        </Col>
                                        <Col md={6}>
                                            <Form.Group className='mb-3'>
                                                <Form.Label><strong>Diagnosis *</strong></Form.Label>
                                                <Form.Control placeholder='Enter diagnosis...' value={prescriptionData.diagnosis} onChange={(e) => handlePrescriptionChange('diagnosis', e.target.value)} required />
                                            </Form.Group>
                                        </Col>
                                        <Col md={6}>
                                            <Form.Group className='mb-3 mt-3'>
                                                <Form.Label><strong>Complain *</strong></Form.Label>
                                                <Form.Control as='textarea' rows={3} placeholder='Enter complain...' value={prescriptionData.complain} onChange={(e) => handlePrescriptionChange('complain', e.target.value)} />
                                            </Form.Group>
                                        </Col>
                                        <Col md={6}>
                                            <Form.Group className='mb-3 mt-3'>
                                                <Form.Label><strong>Past History *</strong></Form.Label>
                                                <Form.Control as='textarea' rows={3} placeholder='Enter past history...' value={prescriptionData.pasHistory} onChange={(e) => handlePrescriptionChange('pasHistory', e.target.value)} />
                                            </Form.Group>
                                        </Col>
                                    </Row>
                                    <Form.Group className='mb-3'>
                                        <Form.Label><strong>Instructions</strong></Form.Label>
                                        <Form.Control as='textarea' rows={2} placeholder='Enter special instructions for patient...' value={prescriptionData.instructions} onChange={(e) => handlePrescriptionChange('instructions', e.target.value)} />
                                    </Form.Group>
                                            ...
                                            <Row className='g-2 mb-3'>
                                                <Col md={2}>
                                                    <Form.Group>
                                                        ...
                                                        <Form.Select value={newPrescriptionItem.type} onChange={(e) => handleNewItemChange('type', e.target.value)}>
                                                            {medicineTypes.map(type => (<option key={type} value={type}>{type.charAt(0).toUpperCase() + type.slice(1)}</option>))}
                                                        </Form.Select>
                                                    </Form.Group>
                                                </Col>
                                                <Col md={2}>
                                                    <Form.Group>
                                                        ...
                                                        <Form.Control type='text' placeholder='e.g. Paracetamol' value={newPrescriptionItem.medicine} onChange={(e) => handleNewItemChange('medicine', e.target.value)} />
                                                    </Form.Group>
                                                </Col>
                                                <Col md={5}>
                                                    <Form.Group>
                                                        ...
                                                        <div className='d-flex flex-wrap gap-2'>
                                                            {[{ id: 'mo', label: 'MO' }, { id: 'an', label: 'AN' }, { id: 'ev', label: 'EV' }, { id: 'nt', label: 'NT' }].map(time => (
                                                                <div key={time.id} className='d-flex align-items-center'>
                                                                    <Form.Check type='checkbox' id={`${time.id}-check`} checked={newPrescriptionItem[time.id]} onChange={(e) => handleNewItemChange(time.id, e.target.checked)} className='me-1' />
                                                                    <Form.Label htmlFor={`${time.id}-check`} className='mb-0 me-2'>{time.label}</Form.Label>
                                                                    <Form.Control type='number' min='0' size='sm' style={{ width: '40px' }} value={newPrescriptionItem[`${time.id}Dose`]} onChange={(e) => handleNewItemChange(`${time.id}Dose`, Math.max(0, e.target.value))} disabled={!newPrescriptionItem[time.id]} />
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </Form.Group>
                                                ...
                                                <Col md={2}>
                                                    <Form.Group>
                                                        ...
                                                        <Form.Select value={newPrescriptionItem.instruction} onChange={(e) => handleNewItemChange('instruction', e.target.value)}>
                                                ...
                                                <Col md={1}>
                                                    <Form.Group>
                                                        ...
                                                        <Form.Control type='number' min='1' value={newPrescriptionItem.days} onChange={(e) => handleNewItemChange('days', Math.max(1, e.target.value))} />
                                                    </Form.Group>
                                                </Col>
                                            </Row>
                                            <div className='d-flex justify-content-between align-items-center mb-3'>
                                                <div><strong>Total Quantity: {newPrescriptionItem.quantity} {newPrescriptionItem.type}s</strong></div>
                                                <Button variant='primary' size='sm' onClick={addPrescriptionItem} disabled={!newPrescriptionItem.medicine.trim() || !['mo', 'an', 'ev', 'nt'].some(t => newPrescriptionItem[t])}>Add to Prescription</Button>
                                            </div>
                                                    ...
                                                    <tbody>
                                                        {prescriptionData.prescriptionItems.map((item, index) => (
                                                            <tr key={index}>
                                                                <td>{item.medicine}</td>
                                                                <td>{item.type}</td>
                                                                <td>{[{ id: 'mo', label: 'MO' }, { id: 'an', label: 'AN' }, { id: 'ev', label: 'EV' }, { id: 'nt', label: 'NT' }].filter(time => item[time.id]).map(time => `${time.label}(${item[`${time.id}Dose`]})`).join(', ')}</td>
                                                                <td>{item.days}</td>
                                                                <td>{item.quantity} {item.type}s</td>
                                                                <td>
                                                                    <Button variant='link' size='sm' className='text-danger p-0' onClick={() => removePrescriptionItem(index)}><MdClose size={18} /></Button>
                                                                </td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
            ...
            >
                ...
                <div style={{ padding: 24, lineHeight: 1.25 }}>
                    <div style={{ background: '#132031', color: '#fff', borderRadius: 10, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ fontWeight: 700, fontSize: 14 }}>
                            BP : <span style={{ fontWeight: 400 }}>{prescriptionData?.bp || '-'}</span>
                        </div>
                    </div>

...
const D_Appointment = () => {
  ...
  const handleContinueChoosePrescription = async () => {
    ...
    if (prescriptionOption === "none") {
      try {
        ...
        Swal.fire(
          "Success",
          "Appointment completed without prescription.",
          "success"
        );
        setShowChoosePrescription(false);
        appointmentlist();
    ...
    if (prescriptionOption === "upload") {
      ...
      try {
        ...
        Swal.fire(
          "Success",
          "Prescription image uploaded and appointment completed.",
          "success"
        );
        setShowChoosePrescription(false);
        setPrescriptionUploadFiles([]);
  ...
  const submitPrescription = async () => {
    ...
    try {
      ...
      const itemsText = (prescriptionData.prescriptionItems || [])
        .map((item) => {
          const times = [];
    ...
    axios({
      ...
      .catch(function (error) {
        ...
        Swal.fire({
          ...
          text:
            error.response?.data?.Message ||
            "This time slot may already be booked or unavailable. Please select a different time.",
          icon: "error",
            ...
            <>
              <OverlayTrigger placement="top" overlay={renderTooltip("Accept")}>
                <button
                  ...
                  onClick={() =>
                    appointmentbtn(row._id, "Accept", row.visit_types)
                  }
                >
                  <MdDone size={18} />
                </button>
              ...
              <OverlayTrigger placement="top" overlay={renderTooltip("Cancel")}>
                <button
                  ...
                  onClick={() =>
                    appointmentbtn(row._id, "Cancel", row.visit_types)
                  }
                >
                  <MdClose size={18} />
                </button>
  ...
  const incomeStats = useMemo(() => {
    if (!appointment) return { currentMonth: 0, last7Days: 0, lastMonth: 0 };
    ...
    appointment.forEach((apt) => {
      ...
      if (apt.status === "Completed" && apt.price) {
        ...
        if (dateParts.length === 3) {
          ...
          // Last 7 days income
          if (aptDate >= sevenDaysAgo && aptDate <= now) {
          ...
          <Col xs={12} lg={9}>
            <div className="appointments-card mb-3 ">
              ...
              <Row className="g-3 mb-4">
                <Col md={4} xs={12}>
                  <Card className="income-widget h-100">
                    <Card.Body className="d-flex flex-column">
                      {/* <div className="d-flex align-items-center justify-content-between mb-2">
                        <div className="income-widget-icon last-7-days">
                          <span className="widget-icon">📊</span>
                        </div>
                        <span className="income-widget-period">Last 7 Days</span>
                      </div> */}
                      <div className="income-widget-amount">
                        ₹{incomeStats.last7Days.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </div>
                      <div className="income-widget-label">Last 7 Days Income</div>
                    </Card.Body>
                  </Card>
                </Col>
                <Col md={4} xs={12}>
                  <Card className="income-widget h-100">
                    <Card.Body className="d-flex flex-column">
                      {/* <div className="d-flex align-items-center justify-content-between mb-2">
                        <div className="income-widget-icon current-month">
                          <span className="widget-icon">📅</span>
                        </div>
                        <span className="income-widget-period">This Month</span>
                      </div> */}
                      <div className="income-widget-amount">
                        ₹{incomeStats.currentMonth.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </div>
                      <div className="income-widget-label">Current Month Income</div>
                    </Card.Body>
                  </Card>
                </Col>
                <Col md={4} xs={12}>
                  <Card className="income-widget h-100">
                    <Card.Body className="d-flex flex-column">
                      {/* <div className="d-flex align-items-center justify-content-between mb-2">
                        <div className="income-widget-icon last-month">
                          <span className="widget-icon">📈</span>
                        </div>
                        <span className="income-widget-period">Last Month</span>
                      </div> */}
                      <div className="income-widget-amount">
                        ₹{incomeStats.lastMonth.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </div>
                      <div className="income-widget-label">Last Month Income</div>
                    </Card.Body>
                  </Card>
                </Col>
              </Row>
                ...
                <div className="bg-light p-3 rounded">
                  ...
                  <div className="row mt-2">
                    ...
                    <div className="col-md-6">
                      ...
                      {currentAppointment.appointment_reason || "Not provided"}
                    </div>
                  </div>
                </div>

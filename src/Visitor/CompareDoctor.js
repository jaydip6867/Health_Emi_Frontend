...
const CompareDoctor = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      setloading(true);
      try {
        const selected = surgeryOptions?.find(
          (s) => s?._id === selectedSurgery
        );
        const res = await axios({
          method: "post",
  ...
  useEffect(() => {
    ...
    if (setSelectedSpecialty || selectedSubSpecialty === 'clear') {
      result = result.filter((doc) => doc.specialty === selectedSpecialty);
      
    }

    if (setSelectedSubSpecialty && selectedSubSpecialty !== 'clear') {
      result = result.filter(
        (doc) => doc.sub_specialty === selectedSubSpecialty
      );
 
    }

    setDoctors(result);
  }, [selectedSubSpecialty, selectedSpecialty]);
  ...
  const secondDoctorMatches = React.useMemo(() => {
    ...
    const filtered = q
      ? base.filter((d) => (d?.name || "").toLowerCase().includes(q))
      : base;
  ...
  const selsurgery = (e) => {
    ...
    var cat_list = categoryOptions?.filter(
      (x) => x?.surgerytypeid?._id === value
    );
              ...
              <Form.Select
                ...
                onChange={(e) => {
                  ...
                  setSelectedSpecialty(
                    selectedOption.getAttribute("data-spacilites")
                  );
                  setSelectedDoctors([]);
                  setSecondDocQuery("");
                  selsurgery(e);
                ...
                {surgeryOptions?.length > 0 &&
                  surgeryOptions?.map((s) => (
                    <option
                      key={s?._id}
                      value={s?._id}
                      data-spacilites={s?.surgerytypename || s?.name}
                    >
          ...
          {selectedSurgery?.length > 0 && (
            <Row className="justify-content-center my-3 g-3">
              <Col xs={12} md={6} lg={5}>
                ...
                >
                  <option value={selectedSpecialty.length > 0 &&  'clear'}> {selectedSpecialty.length > 0 ? 'Unselect Sub Speciality ' : "Select Sub Speciality"}</option>
                  {subSpecialty?.length > 0 &&
                    subSpecialty?.map((item, index) => (
        ...
        <Container>
              ...
              <Card className="p-3 shadow-sm rounded-4 mb-5">
                {(() => {
                  ...
                  const feeText = (d) => {
                    ...
                    if (!cd) return "—";
                    const parts = [
                      cd?.clinic_visit_price
                        ? `Clinic ₹${cd.clinic_visit_price}`
                        : null,
                      cd?.home_visit_price
                        ? `Home ₹${cd.home_visit_price}`
                        : null,
                  ...
                  const surgeriesPieces = (d) => {
                    if (
                      !Array.isArray(d?.surgeriesDetails) ||
                      d.surgeriesDetails.length === 0
                    )
                      return [];
                    return d.surgeriesDetails
                      .map((s) => s?.name || s)
                      .filter(Boolean);
                  };
                  ...
                  const qualificationText = (d) => {
                    return Array.isArray(d?.qualifications) &&
                      d.qualifications.length > 0
                      ? d.qualifications.join(", ")
                      : d?.qualification || "—";
                  };
                  const approvalText = (d) => {
                    if (typeof d?.approval_status !== "undefined")
                      return String(d.approval_status);
                    if (typeof d?.isApproved !== "undefined")
                      return d.isApproved ? "Approved" : "Pending";
                    return "—";
                  };
                  const subSpecialtyText = (d) => {
                    return typeof d?.sub_specialty !== "undefined"
                      ? String(d.sub_specialty)
                      : "—";
                  };
                  return (
                    <div>
                      ...
                      <Row className="align-items-stretch py-3 border-bottom">
                        <Col
                          xs={12}
                          md={3}
                          className="mb-2 mb-md-0 fw-semibold"
                        >
                          Profile
                        </Col>
                          ...
                          {d2 ? (
                            ...
                            <div className="border rounded-3 p-3">
                              ...
                              <Form.Control
                                ...
                                onChange={(e) =>
                                  setSecondDocQuery(e.target.value)
                                }
                                onFocus={() => setSecondDocOpen(true)}
                                onClick={() => setSecondDocOpen(true)}
                                onBlur={() =>
                                  setTimeout(() => setSecondDocOpen(false), 120)
                                }
                                className="rounded-pill mb-2"
                              />
                              {secondDocOpen &&
                                secondDoctorMatches.length > 0 && (
                                  <div
                      ...
                      <Row className="align-items-center py-3 border-bottom">
                        ...
                        <Col xs={12} md={5}>
                          {d2?.experience ? `${d2.experience}` : d2 ? "—" : ""}
                        </Col>
                      </Row>
                      ...
                      <Row className="align-items-center py-3 border-bottom">
                        ...
                        <Col
                          ...
                          className="d-flex align-items-center gap-1"
                        >
                          <MdLocalHospital className="text-danger" />{" "}
                          {hospitalsText(d1)}
                        </Col>
                        <Col
                          xs={12}
          ...
          <section className="mt-4">
            ...
            {doctors.length === 0 ? (
              ...
              <Row className="g-3">
                {doctors.map((doc) => {
                  const isSelected = selectedDoctors.some(
                    (d) => d._id === doc._id
                  );
                  return (
                    <Col xs={6} md={4} lg={3} key={doc._id}>
                      <Card
                        text="secondary"
                        className="my-3 rounded-4 doctor_card doctor-card-v2 overflow-hidden"
                      >
                        <div className="position-relative">
                          <img
                            ...
                            onError={(e) => {
                              ...
                              e.currentTarget.src = defaultDoctorImg;
                            }}
                          />
                          <div className="rating-badge-orange d-inline-flex align-items-center gap-2 position-absolute top-0 start-0 m-3 px-2 py-1 rounded-3">
                            ...
                            <small className="fw-bold text-white fs-7">
                              {doc?.averageRating === 0
                                ? "0"
                                : doc?.averageRating}
                            </small>
                        ...
                        <Card.Body className="pt-3 pb-4 px-0">
                          <div className="d-flex align-items-center justify-content-between mb-2 pe-2">
                            <div
                              className="spec-chip d-inline-flex align-items-center"
                              style={{ width: "60%" }}
                            >
                            ...
                            <div className="chip chip-available">
                              {doc?.is_available === true ? (
                                <small
                                  className="fw-semibold fs-7 px-2 py-1 rounded-3"
                                  style={{
                            ...
                            <Row className="align-items-center g-2 consultation_fees justify-content-center">
                              <Col xs="auto" className="text-end">
                                <button
                                  ...
                                  disabled={
                                    !isSelected && selectedDoctors.length >= 2
                                  }
                                >
                                  {isSelected
                                    ? "Remove"
